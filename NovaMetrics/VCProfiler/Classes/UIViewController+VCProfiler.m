...
static void pmy_viewDidLoad(UIViewController *kvo_self, SEL _sel) {
    ...
    [VCProfilerAdapter.shared.delegate trackViewDidLoad:kvo_self];
    func(kvo_self, _sel);
...
static void pmy_viewWillAppear(UIViewController *kvo_self, SEL _sel, BOOL animated) {
    ...
    [VCProfilerAdapter.shared.delegate trackViewWillAppear:kvo_self];
...
static void pmy_viewDidAppear(UIViewController *kvo_self, SEL _sel, BOOL animated) {
    ...
    [VCProfilerAdapter.shared.delegate trackViewDidAppear:kvo_self];
    func(kvo_self, _sel, animated);
...
- (void)dealloc {
    ...
    if ([_className isEqualToString:NSStringFromClass([_target class])]) {
        [_target removeObserver:[MTHFakeKVOObserver shared] forKeyPath:_keyPath];
    } else {
        NSAssert(NO, @"Class is change. Please check if `object_setClass` is used?");
    }
    _target = nil;
}
...
- (void)createAndHookKVOClass {
    ...
    remover.className = NSStringFromClass([self class]);
